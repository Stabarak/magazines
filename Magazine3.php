...
<html>
	...
	<body style="background-image:url('Do-you-know-your-cars-as-well-as-him.jpg');background-position:center;padding-top:150px;background-repeat:no-repeat;background-size:cover;text-shadow:3px 2px 4px grey;">
		...
		<fieldset style="width:100%;padding:10px;text-align:center" align="center"><legend>Motorcyclist</legend>
			<form action="http://localhost/magazine/Magazine3.php" method="get">
			<div align="center">
...
{
	$query="select * from magz where mid=3";
...
{
		$query="select * from editors where magid=3";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{	
...
elseif(isset($_GET['download']))
{
	...
	readfile('C:/xampp/htdocs/magazine/magazines/Motorcyclist.pdf');
}
elseif(isset($_GET['publisher']))
{
		$query="select * from publisher where magid=3";
	$result=mysqli_query($con,$query);
	while($row=mysqli_fetch_array($result))
	{	
